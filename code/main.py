...
sounds = [pygame.mixer.Sound(f"../assets/sounds/{file}") for file in os.listdir("../assets/sounds")]
...
def update_tool(toolbar: ToolBar, grid: Grid) -> None:
    mouse_pos = pygame.mouse.get_pos()
    if (clicked_tool := toolbar.get_clicked_tool(mouse_pos)) is None:
        return

    toolbar.cur_tool = clicked_tool
    match toolbar.cur_tool.type:
        case ToolType.GOOF:
            goof()
...
def draw_toolbar(toolbar: ToolBar) -> None:
    ...
    screen.blit(toolbar.frame, toolbar.cur_tool.position)
